...
crs = crs %>%
  ...
  unite(full_text, all_of(textual_cols_for_classification), sep=" ", na.rm=T)
...
crs = crs[,c("full_text","limited_text","labels")]
crs = unique(crs)
crs = crs[which(nchar(crs$limited_text) > 5),]

fwrite(crs, "./large_data/meta_model_data_combo.csv")
